# -*- coding: utf-8 -*-
"""Interactive visualiser for Bug1 / Bug2.  Use --planner bug|bug2."""
import sys
import json
import time
import threading
...
# Planner module and function are resolved after CLI parsing (see __main__).
...
def _patched_snapshot(position, scene, message):
    frame = _planner_module.snapshot_frame.__wrapped__(position, scene, message)
...
def _install_patch():
    original = _planner_module.snapshot_frame
    _planner_module.snapshot_frame.__wrapped__ = original
...
class SimThread(threading.Thread):
    ...
    def run(self):
        ...
        result         = _run_fn(raw_scene, max_steps=self.max_steps,
...
def _build_panels(fig, axes, threads, max_steps):
    ...
    for ax, t in zip(axes, threads):
        ...
        panels.append((t, ax, patches, ghost_built, path_line, robot_dot, status_text,
                       DISPLAY_COLORS, max_steps))
    return panels


def _make_update(panels):
    def update(_tick):
        artists = []
        for (t, ax, patches, ghost_built, path_line, robot_dot,
             status_text, DISPLAY_COLORS, max_steps) in panels:
            frames = t.frames
            if not frames:
                artists += [path_line, robot_dot, status_text]; continue
            ...
            arr = np.array([(f.position[0], f.position[1]) for f in frames[:idx + 1]])
            path_line.set_data(arr[:, 0], arr[:, 1])
            robot_dot.set_offsets([[frame.position[0], frame.position[1]]])
            label = ("✓ SUCCESS" if t.success else "✗ FAILED") if t.done else f"step {idx}/{max_steps}"
...
def run_realtime(families, scene_idx, max_steps,
                 step_size=0.07, sensing_range=0.55, save=False, speedup=3):
    threads = [SimThread(f, scene_idx, max_steps, step_size, sensing_range) for f in families]
    for t in threads: t.start()
    while not any(len(t.frames) > 0 for t in threads): time.sleep(0.05)

    n = len(threads)
    fig, axes = plt.subplots(1, n, figsize=(5 * n, 5.5))
    if n == 1: axes = [axes]
    fig.suptitle(f"{_label} — scene {scene_idx:03d}  (live)", fontsize=10)

    panels = _build_panels(fig, axes, threads, max_steps)
    anim   = FuncAnimation(fig, _make_update(panels), interval=60,
    ...
    if save: _save_video(threads, scene_idx, speedup=speedup)


# ── Batch mode ────────────────────────────────────────────────────────────────

def run_batch(families, scene_indices, max_steps,
              step_size=0.07, sensing_range=0.55, save=False,
              speedup=3, max_workers=8):
    import concurrent.futures

    def _run_scene(scene_idx):
        threads = [SimThread(f, scene_idx, max_steps, step_size, sensing_range)
                   for f in families]
        for t in threads: t.start()
        for t in threads: t.join()
        _save_log(threads, scene_idx)
    ...
    print(f"{_label} batch — {len(scene_indices)} scenes  families: {', '.join(families)}")
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as ex:
        futures = {ex.submit(_run_scene, idx): idx for idx in scene_indices}
        for fut in concurrent.futures.as_completed(futures):
            scene_idx, results = fut.result()
            summary = "  ".join(f"{f[:3]}={'✓' if ok else '✗'}" for f, ok in results.items())
...
def _save_log(threads, scene_idx):
    ...
    log_path  = log_dir / f"{_label.lower()}_scene{scene_idx:03d}_{timestamp}.txt"
    with open(log_path, "w") as f:
        f.write(f"{_label} Simulation Log\n")
...
def _save_video(threads, scene_idx, speedup=3, fps=30):
    ...
    fig, axes = plt.subplots(1, n, figsize=(5 * n, 5.5))
    if n == 1: axes = [axes]
    fig.suptitle(f"{_label} — scene {scene_idx:03d}", fontsize=10)
    ...
    def update(fi):
        ...
        for t, patches, path_line, robot_dot, status_text in panels:
            ...
            for patch, obs in zip(patches, frame.obstacles):
                ...
                patch.set_alpha(0.92 if obs["observed"] else 0.42)
                patch.set_edgecolor("#111111" if obs["observed"] else "#666666")
            arr = np.array([(f.position[0], f.position[1]) for f in t.frames[:idx + 1]])
            path_line.set_data(arr[:, 0], arr[:, 1])
            robot_dot.set_offsets([[frame.position[0], frame.position[1]]])
            done = idx >= len(t.frames) - 1
            status_text.set_text(("✓ SUCCESS" if t.success else "✗ FAILED") if done else f"step {idx}")
    ...
    out_path = out_dir / f"{_label.lower()}_scene{scene_idx:03d}.mp4"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Interactive Bug1/Bug2 visualiser.")
    parser.add_argument("--planner", choices=["bug", "bug2"], default="bug",
                        help="Which bug algorithm to run (default: bug)")
    parser.add_argument("--scene",   type=int, nargs="+", default=[0])
    parser.add_argument("--scenes",  type=str, default=None,
                        help="Range e.g. '0-99'. Implies batch mode.")
    ...
    # Resolve planner at runtime so the patch targets the right module
    if args.planner == "bug":
        import scholar.algorithms.bug as _planner_module
        from scholar.algorithms.bug import run_bug as _run_fn
        _label = "Bug1"
    else:
        import scholar.algorithms.bug2 as _planner_module
        from scholar.algorithms.bug2 import run_bug2 as _run_fn
        _label = "Bug2"

    _install_patch()

    families      = args.family or FAMILIES
    scene_indices = list(range(*map(int, args.scenes.split("-")[:2]), 1)) \
    ...
    if len(scene_indices) > 1 or args.scenes:
        run_batch(families, scene_indices, args.steps, args.step, args.sense,
                  args.save, args.speedup, args.workers)
    else:
        print(f"{_label} — scene {scene_indices[0]:03d}  families: {', '.join(families)}")
        run_realtime(families, scene_indices[0], args.steps, args.step, args.sense,
                     args.save, args.speedup)
